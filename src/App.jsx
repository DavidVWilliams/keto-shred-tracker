...
// --- Firebase Initialization (Secured via Environment Variables) ---
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen text-slate-800 flex flex-col font-sans pb-12 shadow-xl border-x border-[#e2e2e2] bg-white">
      ...
      <header className="bg-white px-5 pt-4 pb-3 border-b border-[#eaeaea] sticky top-0 z-30">
        ...
        {/* Tab Navigation: Dashboard is opening page */}
              ...
              <div className="flex gap-2 pt-1">
                <button 
                  ...
                  onClick={() => {
                    ...
                    setWeightHistory(resetHistory);
                    syncToCloudAndLocal({ weightData: resetWeight, weightHistory: resetHistory });
                    setToastMessage('Weight profile reset.');
                    setTimeout(() => setToastMessage(null), 3000);
                    setModalType(null);
                  }}
                ...
                <button 
                  ...
                  onClick={() => {
                    if (weightData.current > 0) {
                      ...
                      const diff = lastEntry ? Number((weightData.current - lastEntry.weight).toFixed(1)) : 0;
                      const newEntry = {
                        id: `wh-${Date.now()}`,
                      ...
                      syncToCloudAndLocal({ weightData, weightHistory: updatedHistory });
                      setToastMessage('Weight logged & profile saved!');
                    } else {
                      syncToCloudAndLocal({ weightData });
                      setToastMessage('Weight profile saved!');
                    }
                    setTimeout(() => setToastMessage(null), 3000);
                    setModalType(null);
                  }}
