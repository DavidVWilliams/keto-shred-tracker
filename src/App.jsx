...
export default function App() {
  ...
  const syncToCloud = async (updatedState) => {
    if (!user) return;
    try {
  ...
  if (authLoading) {
    return (
      <div className="max-w-md mx-auto min-h-screen bg-white flex items-center justify-center p-6 text-slate-800 font-sans">
        <div className="text-center">
          <div className="w-8 h-8 border-4 border-slate-300 border-t-[#82bc41] rounded-full animate-spin mx-auto mb-3" />
          <p className="text-xs font-bold text-slate-700">Loading Keto Shred Tracker...</p>
        </div>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="max-w-md mx-auto min-h-screen bg-white flex flex-col items-center justify-center p-6 text-slate-800 font-sans border-x border-[#e2e2e2] shadow-xl">
        <div className="w-full bg-white rounded-3xl border border-[#eaeaea] p-8 text-center shadow-lg">
          <div className="w-12 h-12 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center justify-center mx-auto mb-4 text-[#82bc41]">
            <Icons.Flame size={24} />
          </div>
          <h1 className="text-xl font-black text-slate-900 mb-1 uppercase tracking-tight">Keto Shred Tracker</h1>
          <p className="text-xs font-bold text-slate-700 mb-6">Joint-Friendly Fat Loss • 61yo Protocol</p>
          
          <button
            onClick={handleGoogleSignIn}
            className="w-full py-3.5 px-4 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-black text-xs flex items-center justify-center gap-2 shadow-md transition-all"
          >
  ...
  const featuredIndex = Math.abs(hash) % recipes.length;
  const currentFeaturedDish = recipes[featuredIndex];
  const isFeaturedInLibrary = recipes.some(r => r.name === currentFeaturedDish.name);

  const handleDeleteRecipe = (recipeId) => {
    const updated = recipes.filter(r => r.id !== recipeId);
    setRecipes(updated);
    syncToCloud({ recipes: updated });
  ...
  const defaultDay = INITIAL_SCHEDULE.find(d => d.id === selectedDay) || INITIAL_SCHEDULE[0];
  ...
  const toggleTask = (taskId) => {
    ...
    setCompletion(updatedComp);
    syncToCloud({ completion: updatedComp });
  };
  ...
  const weeklyBdpMinutes = INITIAL_SCHEDULE.reduce((total, d) => {
  ...
  const handleRemoveWorkoutFromDay = (workoutId) => {
    ...
    syncToCloud({ customDays: updatedCustomDays });
  };
  ...
  const handleAddLibraryWorkoutToDay = () => {
    ...
    const updatedWorkouts = [...currentWorkouts, newDayWorkout];
    const updatedCustomDays = {
      ...customDays,
      [selectedDay]: { ...(customDays[selectedDay] || defaultDay), workouts: updatedWorkouts }
    };
  ...
  const handleCreateNewGlobalWorkout = (e) => {
    ...
    const updatedLib = [...workoutLibrary, newLibItem];
    setWorkoutLibrary(updatedLib);
    syncToCloud({ workoutLibrary: updatedLib });
  ...
  const handleDeleteGlobalWorkout = (libId) => {
    const updatedLib = workoutLibrary.filter(w => w.id !== libId);
    setWorkoutLibrary(updatedLib);
    syncToCloud({ workoutLibrary: updatedLib });
      ...
      <header className="bg-white px-5 pt-4 pb-3 border-b border-[#eaeaea] sticky top-0 z-30">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-[10px] font-bold tracking-wider mt-0.5 text-slate-700">Joint-Friendly Fat Loss • 61yo Protocol</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="text-[8px] text-slate-700 font-bold uppercase tracking-wider">Lost</span>
            </button>
            <button 
              onClick={handleSignOut}
              className="p-2 rounded-xl border border-[#e2e2e2] bg-white text-slate-700 hover:text-red-600 transition-colors"
              title="Sign Out"
            >
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 py-2.5 text-xs font-bold transition-all border-b-2 -mb-[1px] ${
                activeTab === tab.id ? 'text-slate-900' : 'text-slate-700 border-transparent hover:text-slate-900'
              }`}
                ...
                return (
                  <button
                    ...
                    className={`py-3 text-center flex flex-col items-center justify-center transition-all border-r border-[#eaeaea] last:border-r-0 ${
                      isSelected ? 'bg-white font-bold' : 'hover:bg-slate-100/60 text-slate-700'
                    }`}
            ...
            <div className="p-5 space-y-6">
              ...
              <div className="flex justify-between items-center pb-4 border-b border-[#eaeaea]">
                ...
                <div className="text-right flex items-center gap-3">
                  <div>
                    ...
                    <span className="text-[9px] text-slate-700 font-bold uppercase">{targetProtein}g Protein</span>
                  </div>
                  <button 
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Training Routine</span>
                  <div className="flex gap-2">
                    <button 
                      onClick={() => setModalType('manageWorkouts')} 
                      className="text-[10px] font-extrabold text-slate-800 hover:text-slate-950 bg-slate-100 px-2 py-1 rounded-lg border border-slate-300"
                    >
                      Library
                    </button>
                    <button 
                ...
                <div className="space-y-2">
                    ...
                    return (
                      <div 
                        ...
                        className={`flex items-start justify-between p-3 rounded-xl border transition-all ${
                          isChecked ? 'bg-slate-50 border-slate-300 opacity-50' : 'bg-white border-[#eaeaea]'
                        }`}
                      >
                        <label className="flex items-start gap-3 cursor-pointer flex-1 mr-2">
                          <input 
                            ...
                            className="mt-0.5 w-4 h-4 rounded border-slate-400"
                            style={{ accentColor: '#82bc41' }} 
                          />
                          <div>
                            <span className={`text-xs font-bold block ${isChecked ? 'text-slate-700 line-through' : 'text-slate-900'}`}>
                              {w.name}
                            </span>
                            {w.url && (
                              <a 
                                ...
                                target="_blank" 
                                rel="noreferrer" 
                                className="inline-flex items-center gap-1 mt-1 text-[9px] font-bold text-slate-800 hover:text-slate-950"
                              >
                        ...
                        <button onClick={() => handleRemoveWorkoutFromDay(w.id)} className="text-slate-500 hover:text-red-500">
                          <Icons.Trash size={13} />
                        </button>
                      </div>
                    );
                  ...
                  {currentWorkouts.length === 0 && (
                    <div className="p-3 text-center text-xs text-slate-700 bg-[#fafafa] rounded-xl border border-dashed border-[#eaeaea]">
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Nutrition & Meals</span>
                        ...
                        return (
                          ...
                          >
                            <label className="flex items-center gap-3 cursor-pointer flex-1 mr-2">
                              ...
                              />
                              <div>
                                <span className={`text-xs font-bold block ${isChecked ? 'line-through text-slate-700' : 'text-slate-900'}`}>
                                  {m.name}
                                </span>
                            ...
                            <button onClick={() => handleRemoveMeal(m.id)} className="text-slate-500 hover:text-red-500">
                              <Icons.Trash size={13} />
                            </button>
                          </div>
                        );
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-3">
            ...
            <div className="bg-white rounded-2xl border border-[#eaeaea] p-5 shadow-xs">
              <div className="flex justify-between items-center mb-3">
                <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Weight Loss Goal</span>
              ...
              <div className="flex items-baseline justify-between mb-2">
                <div>
                  ...
                  <span className="text-xs text-slate-700 font-bold ml-1">lbs</span>
                </div>
                <div className="text-xs text-slate-800 font-bold">
                  Target: {weightData.goal} lbs
                </div>
              </div>
              <div className="w-full bg-slate-300 h-2 rounded-full overflow-hidden mb-2">
            ...
            <div className="bg-white rounded-2xl border border-[#eaeaea] p-5 shadow-xs">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Weekly BDP Goal</span>
              ...
              {weeklyBdpMinutes >= 80 ? (
                <div className="text-xs font-bold text-emerald-700">✓ 80-Minute Weekly Burpee Benchmark Reached!</div>
              ) : (
                <p className="text-[11px] text-slate-800">Complete your four 20-minute sessions to hit your weekly conditioning target.</p>
              )}
            </div>

            {/* Featured Dish Banner (Dark Slate Background Restored) */}
                ...
                {!isFeaturedInLibrary && (
                  <button
                    onClick={() => {
                      ...
                      syncToCloud({ recipes: updatedRecipes });
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 ${
                    recipeCategory === cat ? 'text-white shadow-xs' : 'bg-white border border-[#eaeaea] text-slate-800 hover:bg-slate-50'
                  }`}
              ...
              {filteredRecipes.map(recipe => (
                <div key={recipe.id} className="bg-white rounded-2xl border border-[#eaeaea] p-4 shadow-xs">
                  <div className="flex justify-between items-start mb-1.5">
                    <div>
                      <span className="text-[9px] font-black uppercase tracking-wider text-slate-700 block mb-0.5">
                        {recipe.category}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className="text-slate-800 hover:text-slate-950 p-1 bg-slate-100 rounded-lg border border-slate-300 text-[10px] font-bold px-2 py-1 flex items-center gap-1"
                      >
                        View <Icons.ExternalLink size={11} />
                      </button>
                      <button 
                        onClick={() => handleDeleteRecipe(recipe.id)}
                        className="text-slate-500 hover:text-red-600 p-1 bg-slate-100 rounded-lg border border-slate-300"
                        title="Delete Recipe"
                      >
                        <Icons.Trash size={12} />
                      </button>
                  ...
                  <div className="flex gap-2 text-[10px] font-bold text-slate-700 uppercase tracking-wider mb-2">
                    <span style={{ color: '#82bc41' }}>{recipe.cals} KCAL</span>
                    <span>• {recipe.protein}g PRO</span>
                    <span>• {recipe.carbs}g CARB</span>
                  </div>

                  <p className="text-xs text-slate-800 bg-[#fafafa] p-2.5 rounded-xl border border-[#eaeaea] leading-relaxed line-clamp-2">
                    {recipe.instructions}
                  </p>
                </div>
              ))}
        ...
        {activeTab === 'fasting' && (
          <div className="space-y-3">
            <div className="bg-white rounded-2xl border border-[#eaeaea] p-6 shadow-xs text-center">
              ...
              <h2 className="text-[10px] font-bold text-slate-700 uppercase tracking-widest mb-1">
              ...
              <div className="text-xs font-bold text-slate-800 mb-5">
                    ...
                    {[18, 36, 48].map(hrs => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          syncToCloud({ fastingState: updated });
                        }}
                        className={`py-2 rounded-xl text-xs font-bold border transition-colors ${
                          fastingState.preset === hrs ? 'text-white border-transparent' : 'bg-white text-slate-800 border-[#eaeaea]'
                        }`}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md border border-[#eaeaea] shadow-2xl max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-3">
              ...
              <button onClick={() => setActiveRecipeModal(null)} className="text-slate-500 hover:text-slate-950">
                <Icons.X size={18} />
              </button>
            </div>

            <div className="flex gap-3 text-xs font-extrabold text-slate-900 mb-4 bg-slate-100 p-2.5 rounded-xl border border-slate-300">
            ...
            <div className="space-y-3 mb-5">
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 block mb-1">Ingredients</span>
                <ul className="list-disc pl-4 text-xs text-slate-900 space-y-1 font-medium bg-slate-50 p-3 rounded-2xl border border-slate-300">
              ...
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 block mb-1">Cooking Instructions</span>
                <p className="text-xs text-slate-900 leading-relaxed bg-slate-50 p-3 rounded-2xl border border-slate-300 font-medium">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-900 text-xs font-bold rounded-xl border border-slate-300"
              >
                Close
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-xs z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm border border-[#eaeaea] shadow-xl">
            <div className="flex justify-between items-center mb-3">
              ...
              <button onClick={() => setModalType(null)} className="text-slate-500 hover:text-slate-950">
                <Icons.X size={15} />
              </button>
            </div>
            <form onSubmit={handleSaveGoals} className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-700 uppercase">Target Calories (kcal)</label>
                <input 
                  type="number" 
                  name="cals" 
                  defaultValue={targetCals}
                  className="w-full p-2.5 rounded-xl border border-slate-300 bg-[#fafafa] font-bold text-slate-900 text-xs mt-1 focus:outline-none" 
                />
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-700 uppercase">Target Protein (grams)</label>
                <input 
                  type="number" 
                  name="protein" 
      ...
      {modalType === 'workout' && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-xs z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm border border-[#eaeaea] shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-700 uppercase block mb-1">Choose from Master Library</label>
                <select 
                  value={selectedWorkoutIdToAdd}
                  onChange={(e) => setSelectedWorkoutIdToAdd(e.target.value)}
                  className="w-full p-2.5 rounded-xl border border-slate-300 text-xs font-bold text-slate-900 bg-[#fafafa] focus:outline-none"
                >
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-xs z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm border border-[#eaeaea] shadow-xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-2 mb-4">
              {workoutLibrary.map(w => (
                <div key={w.id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl border border-slate-300">
                  <div>
                    <span className="text-xs font-bold block text-slate-900">{w.name}</span>
                    <span className="text-[9px] text-slate-700">{w.minutes} minutes</span>
                  </div>
                  <button onClick={() => handleDeleteGlobalWorkout(w.id)} className="text-slate-500 hover:text-red-600 p-1">
                    <Icons.Trash size={13} />
                  </button>
                </div>
              ))}
            ...
            <form onSubmit={handleCreateNewGlobalWorkout} className="space-y-2.5">
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-800 block">Add New Global Workout</span>
              <div>
                <input 
                  type="text" 
                  ...
                  className="w-full p-2.5 rounded-xl border border-slate-300 bg-slate-50 text-xs font-bold text-slate-900 focus:outline-none"
                />
              </div>
      ...
      {modalType === 'meal' && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-xs z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm border border-[#eaeaea] shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-700 uppercase block mb-1">Choose Recipe</label>
                <select 
      ...
      {modalType === 'weight' && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-xs z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm border border-[#eaeaea] shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-700 uppercase">Starting Weight (lbs)</label>
                <input 
                  type="number" 
                  value={weightData.start}
                  onChange={(e) => setWeightData(prev => ({ ...prev, start: parseFloat(e.target.value) || 0 }))}
                  className="w-full p-2.5 rounded-xl border border-slate-300 bg-[#fafafa] font-bold text-slate-900 text-xs mt-1 focus:outline-none"
                />
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-700 uppercase">Current Weight (lbs)</label>
                <input 
                  type="number" 
                  step="0.1"
                  value={weightData.current}
                  onChange={(e) => setWeightData(prev => ({ ...prev, current: parseFloat(e.target.value) || 0 }))}
                  className="w-full p-2.5 rounded-xl border border-slate-300 bg-[#fafafa] font-bold text-slate-900 text-xs mt-1 focus:outline-none"
                />
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-700 uppercase">Goal Weight (lbs)</label>
                <input 
                  type="number" 
                  value={weightData.goal}
                  onChange={(e) => setWeightData(prev => ({ ...prev, goal: parseFloat(e.target.value) || 0 }))}
                  className="w-full p-2.5 rounded-xl border border-slate-300 bg-[#fafafa] font-bold text-slate-900 text-xs mt-1 focus:outline-none"
                />
              </div>
              <button 
                onClick={() => {
                  syncToCloud({ weightData });
                  setModalType(null);
                }}
